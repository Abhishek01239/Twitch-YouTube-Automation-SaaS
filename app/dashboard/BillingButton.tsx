...
export default function BillingButton({channelId}:{channelId:string}){
  ...
  async function subscribe(){
    ...
    try{
      const response=await fetch("/api/billing/subscribe",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({channelId})});
  ...
  return <div className="mt-3">
    <button onClick={subscribe} disabled={loading} className="rounded-lg bg-white px-4 py-2 text-sm font-semibold text-black disabled:opacity-50">
